...
class InferManager(base.InferManager):
    ...
    def process_single_file(self, wsi_path, msk_path, output_dir):
        ...
        if np.sum(self.wsi_mask) == 0:
            ...
            return
        if self.save_mask:
            cur_mask_path = os.path.join(output_dir, "mask", "{}.png".format(wsi_name))
            cv2.imwrite(cur_mask_path, self.wsi_mask * 255)
        if self.save_thumb:
            wsi_thumb_rgb = self.wsi_handler.get_full_img(read_mag=scaled_wsi_mag)
            cur_thumb_path = os.path.join(output_dir, "thumb", "{}.png".format(wsi_name))
            cv2.imwrite(cur_thumb_path, cv2.cvtColor(wsi_thumb_rgb, cv2.COLOR_RGB2BGR))
        ...
        if self.save_mask or self.save_thumb:
            json_path = os.path.join(output_dir, "json", "{}.json".format(wsi_name))
        else:
            json_path = os.path.join(output_dir, "{}.json".format(wsi_name))
    ...
    def process_wsi_list(self, run_args):
        ...
        thumb_output_dir = os.path.join(self.output_dir, "thumb")
        if not os.path.exists(thumb_output_dir):
            os.makedirs(thumb_output_dir)
        mask_output_dir = os.path.join(self.output_dir, "mask")
        if not os.path.exists(mask_output_dir):
            os.makedirs(mask_output_dir)
        # process slides one-by-one
        wsi_path_list = glob.glob(self.input_dir + "/*.tiff")
        wsi_path_list.sort()  # ensure ordering
        for iw, wsi_path in enumerate(wsi_path_list):
            cur_time_str = datetime.now(pytz.timezone('America/Chicago')).strftime("%H:%M:%S")
            print("@ {} Start {:2d}/{:2d} on {}".format(cur_time_str, iw + 1, len(wsi_path_list), wsi_path))
            wsi_base_name = pathlib.Path(wsi_path).stem
            msk_path = os.path.join(self.input_mask_dir, "{}.png".format(wsi_base_name))
            if self.save_thumb or self.save_mask:
                output_file = os.path.join(json_output_dir, "{}.json".format(wsi_base_name))
            else:
                output_file = os.path.join(self.output_dir, "{}.json".format(wsi_base_name))
