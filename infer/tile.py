...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        self.name_path_dict = {}
        file_path_list = []
        for root, _, fnames in sorted(os.walk(self.input_dir)):
            for fname in sorted(fnames):
                if fname.endswith(".png"):
                    cur_path = os.path.join(root, fname)
                    file_path_list.append(cur_path)
                    self.name_path_dict[os.path.splitext(fname)[0]] = root
        ...
        def proc_callback(results):
            ...
            # save cell segmenation results
            result_save_dir = self.name_path_dict[img_name].replace(self.input_dir, self.output_dir)
            if not os.path.exists(result_save_dir):
                os.makedirs(result_save_dir)
            # # save mat results
            # mat_save_dir = os.path.join(result_save_dir, "mat")
            # if not os.path.exists(mat_save_dir):
            #     os.makedirs(mat_save_dir)
            # sio.savemat(os.path.join(mat_save_dir, "{}.mat".format(img_name)), mat_dict)
            # # save overlay results
            # overlay_save_dir = os.path.join(result_save_dir, "overlay")
            # if not os.path.exists(overlay_save_dir):
            #     os.makedirs(overlay_save_dir)
            # cv2.imwrite(os.path.join(overlay_save_dir, "{}.png".format(img_name)), cv2.cvtColor(overlaid_img, cv2.COLOR_RGB2BGR))
            # save json results
            # json_save_dir = os.path.join(result_save_dir, "json")
            # if not os.path.exists(json_save_dir):
            #     os.makedirs(json_save_dir)
            self.__save_json(os.path.join(result_save_dir, "{}.json".format(img_name)), inst_info_dict, None)
            return img_name
